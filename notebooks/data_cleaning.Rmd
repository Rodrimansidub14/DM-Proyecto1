...
```{r setup, include=TRUE, echo=TRUE, results='markup'}
library(tidyverse)
library(lubridate)
library(stringr)
```
...
```{r load_data, echo=TRUE, results='markup'}
movies_df <- tryCatch(
    {
        read_csv(
...
```{r initial_checks, echo=TRUE, results='markup'}
...
```{r handle_duplicates, echo=TRUE, results='markup'}
...
```{r handle_missing_values, echo=TRUE, results='markup'}
movies_df <- movies_df %>%
    mutate(
...
```{r clean_columns, echo=TRUE, results='markup'}
movies_df <- movies_df %>%
    mutate(
...
```{r split_columns, echo=TRUE, results='markup'}
movies_df <- movies_df %>%
...
```{r clean_text, echo=TRUE, results='markup'}
movies_df <- movies_df %>%
...
```{r filter_extreme_values, echo=TRUE, results='markup'}
...
```{r final_checks, echo=TRUE, results='markup'}
