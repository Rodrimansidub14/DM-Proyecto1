...
```{r 1, echo=FALSE}
# Filtrar: Excluir películas sin ingresos (revenue NA o 0)
movies_top10 <- movies_cleaned %>%
  filter(!is.na(revenue), revenue > 0) %>%  # Excluir NA y revenue 0
  arrange(desc(revenue)) %>%                # Ordenar de mayor a menor según "revenue"
  head(10) %>%                            # Seleccionar las 10 películas con más ingresos
  arrange(revenue)                        # Reordenar de menor a mayor para el gráfico

# Seleccionar y mostrar las columnas relevantes:
# En este caso se muestran 'title', 'revenue', 'director' y 'releaseDate'
movies_top10_relevant <- movies_top10 %>%
  select(title, revenue, director, releaseDate)

# Imprimir el subconjunto de datos con las columnas relevantes
movies_top10_relevant
...
```{r 2, echo=TRUE, message=FALSE, warning=FALSE, fig.width=8, fig.height=5}
...
# Filtrar datos: solo películas con ingresos relevantes (revenue > 0)
data_marketing <- movies_cleaned %>%
  filter(!is.na(revenue), revenue > 0)

# Crear variables indicadoras:
# - 'has_homepage': "Con Página Oficial" si homePage no es NA ni cadena vacía, y "Sin Página Oficial" en caso contrario.
# - 'has_video': "Con Video Promocional" si la variable video es TRUE, y "Sin Video Promocional" si es FALSE.
data_marketing <- data_marketing %>%
  mutate(
    has_homepage = if_else(is.na(homePage) | homePage == "", "Sin Página Oficial", "Con Página Oficial"),
    has_video = if_else(video, "Con Video Promocional", "Sin Video Promocional")
  )

# Resumen de ingresos por estrategia de página oficial
summary_homepage <- data_marketing %>%
...
print("Resumen de ingresos según Página Oficial:")
print(summary_homepage)
```

```{r 11, echo=FALSE, warning=FALSE}
library(gridExtra)

# Resumen de ingresos por estrategia de video promocional
summary_video <- data_marketing %>%
  group_by(has_video) %>%
  summarise(
    avg_revenue = mean(revenue, na.rm = TRUE),
    median_revenue = median(revenue, na.rm = TRUE),
...
print("Resumen de ingresos según Video Promocional:")
print(summary_video)

# Gráfico 1: Distribución de ingresos según presencia de Página Oficial
p1 <- ggplot(data_marketing, aes(x = has_homepage, y = revenue, fill = has_homepage)) +
  geom_boxplot() +
  labs(title = "Ingresos según Estrategia: Página Oficial",
       x = "Estrategia de Página Oficial",
       y = "Ingresos (revenue)") +
  scale_y_continuous(labels = dollar_format(scale = 1e-6, suffix = "M")) +
  theme_minimal() +
